	...
	foreach($items as $item) {
		...
		foreach($fields as $key => $field) {
			...
			if($key == 'title') {
				...
				$str_moduleitems .= "<td class='item_title' id='title_{$item['id']}'>{$attach}{$picture}<a href=\"index.php?file=admincp&action=edititem&id={$item['id']}\">{$title}</a>$page $draft</td>";
			} elseif($key == 'category') {
				$str_moduleitems .= "<td id='category_{$item['id']}'>{$category}</td>";
			} elseif($key == 'section') {
				$str_moduleitems .= "<td id='section_{$item['id']}'>{$section}</td>";
			} elseif($key == 'dateline') {
				$str_moduleitems .= "<td id='dateline_{$item['id']}' class='mininum' title='".date('Y-m-d H:i:s', $item['dateline'])."'>".date('Y-m-d', $item['dateline'])."</td>";
			} else {
				if(!empty($_d[$key])) {
		...
		$str_items .= "<tr id='item_{$item['id']}'><td>{$checkbox}&nbsp;{$item['id']}</td>{$str_moduleitems}</td><td align='center'><a href='index.php?file=admincp&action=deleteitem&id={$item['id']}&vc={$vc}' onclick='return confirmdelete()'>".alert($lan['delete'])."</a></td>";
		if($data['page'] > 0) {
