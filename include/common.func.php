...
function ak_substr($str, $start, $len = 0xFFFFFFFF, $strip = '', $charset_force = '') {
	...
	if($start + $new_length < $old_length) {
		$return .= $strip;
	}
	return $return;
}
...
function gbk_substr($str, $start, $len=0xFFFFFFFF) {
	...
	$begin = 0;
	$subLen = 0;
	for($i = $start; $i < $start + $len && $i < $strlen; $i++) {
		if($i < $start) {
...
function utf8_substr($str, $start, $len) {
	...
	for($i = 0; $i < $start + $len; $i++) {
		...
		if(ord($temp_str) > 127) {
			$i ++;
			if($i < $start + $len) {
				if($i > $start) $new_str[] = substr($str, 0, 3);
				$str = substr($str, 3);
			}
		} else {
			if($i > $start) $new_str[] = substr($str, 0, 1);
			$str=substr($str,1);
		}
	}
	return join($new_str);
...
function mysql_createtable($key, $data) {
	...
	foreach($data['fields'] as $k => $v) {
		if($v['type'] == 'text' || $v['type'] == 'mediumtext' || $v['type'] == 'longtext') {
			$sql .= "`$k` longtext NOT NULL";
		} else {
			if($v['type'] == 'float') {
				$sql .= "`$k` {$v['type']}";
			} else {
		...
		if(function_exists("curl_file_create")) {
			foreach($params as $k => $v) {
				if(strlen($v) > 2 && substr($v, 0, 1) == '@') {
					$filename = substr($v, 1);
					$media = curl_file_create($filename, 'image/jpeg', 'tmp');
					$params[$k] = $media;
				}
			}
		}
...
function renderinput($name, $type = 'string', $standby = '', $value = '', $style = '') {
	...
	if($type == 'string') {
		...
		$input = '<input type="text" name="'.$name.'" value="'.ak_htmlspecialchars($value).'" style="'.$style.'size="15">';
	} elseif($type == 'pass') {
		$input = '<input type="password" name="'.$name.'" value="'.$value.'" size="50" style="'.$style.'>';
		...
		foreach($options as $_value => $_text) {
			...
			$key = md5($_value);
			$input .= "<input style='$style' type='$type' name='{$name}[]' id='{$name}_{$key}' value='$_value'$checked>&nbsp;<label for='{$name}_{$key}'>$_text</label>&nbsp;";
		}
	} elseif($type == 'text') {
		if($style == '') $style = 'width:500px;height:100px;';
		$input = "<textarea name='$name' style='$style'>$value</textarea>";
	} elseif($type == 'richtext') {
		$input = editor($name, 'rich', $value);
	} elseif($type == 'category') {
		$input = "<select id='$name' name='{$name}' style='$style'>".get_select('category').'</select><script>$(document).ready(function(){$("#'.$name.'").val('.$value.');});</script>';
	} elseif($type == 'categories') {
		if($style == '') $style = 'height:88px;';
		$input = "<select id='$name' name='{$name}[]' multiple style='$style'>".get_select('category');
...
function ifinlist($string, $findme, $delimiter = ',') {
	$lists = explode($delimiter, $string);
	foreach($lists as $item) {
		$item = trim($item);
		if($item == '') continue;
		if($item == $findme) return true;
	}
	return false;
}

function ifmatchlist($string, $match, $delimiter = ',') {
	if($match == '') return false;
	$lists = explode($delimiter, $match);
	foreach($lists as $item) {
		$item = trim($item);
...
function ifstartwith($a, $b) {
	if($a == '' || $b == '') return false;
	$len_a = strlen($a);
	$len_b = strlen($b);
	if($len_a < $len_b) return false;
	if(substr($a, 0, $len_b) == $b) return true;
	return false;
}
...
function file_gbktoutf8($file, $ext = '', $skip = '') {
	
	
	if(is_file($file)) {
		$files = array();
	} else {
		$files = readpathtoarray($file);
	}
	foreach($files as $file) {
		if(ifmatchlist($file, $skip)) continue;
		if(is_dir($file)) {
			file_gbktoutf8($file, $ext, $skip);
		} else {
			if(ifextwith($file, $ext)) {
...
function ifextwith($filename, $exts) {
	if($exts == '') return true;
	$ext = fileext($filename);
	$exts = explode(',', $exts);
...
function regtext($text) {
	if(preg_match("/^(.*)\[:([0-9]+)\]$/", $text, $match)) {
		...
		$suffix = $match[2];
		return ak_substr($text, 0, $suffix);
	} elseif(preg_match("/^(.*)\[#([0-9a-z_]{0,20})\]$/", $text, $match)) {
		return filter($match[2], $match[1]);
	} elseif(preg_match("/^(.*)\[@([0-9a-z_]{0,20})\]$/", $text, $match)) {
		$suffix = $match[2];
		...
		$exe = $match[2];
		$exe = str_replace('$1', "'".addslashes($match[1])."'", $exe);
		return eval('return '.$exe.';');
	}
	return $text;
}
?>