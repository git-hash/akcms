...
function operatehtml($html, $params, $starttime) {
	global $ifdebug;
	$_html = $html;
	if($_html != '' && isset($params['startwith'])) $html = $params['startwith'].$html;
	if($_html != '' && isset($params['endwith'])) $html = $html.$params['endwith'];
	$endtime = akmicrotime();
...
function getitemsdata($params) {
	...
	$flags = getitems_parseflag($params);
	$params['fields'][] = 'categoryid';
	...
	if(!empty($ids)) {
		...
		if($f_data) {
			...
			foreach($texts as $text) {
				...
				if($f_datahighlight) $items[$text['itemid']]['data_highlight'] = highlight($items[$text['itemid']]['data'], $params['keywords']);
			}
		}
	}
	$j = 0;
	...
	foreach($items as $item) {
		...
		$item['module'] = $categories[$c]['module'];
		$template = getitems_parsetemplate($item, $params);
		...
		if($f_titlekeywords) $item['title_keywords'] = renderkeywords($item['title'], $item['keywords']);
		...
		if($f_titlehighlight) $item['title_highlight'] = highlight($item['title'], $params['keywords']);
		...
		if(!empty($item['picture'])) {
			if($f_picture) {
				$_picture = getfield('[picture:', ']', $template);
		...
		if(strpos($params['template'], '<#ak_if') !== false || !empty($params['allfield'])) {
			$_item = array_merge($item, $_item);
		} else {
			foreach($params['fields'] as $k) {
				...
				if(isset($item[$k])) $_item[$k] = $item[$k];
			}
		}
		$_item['template'] = $template;
		hookfunction('getitemsfield', $_item, $params);
		$datas[] = $_item;
	}
	return $datas;
}
...
function getcategoriessql($params) {
	...
	if($params['rootcategory'] >= 0) {
		if(empty($params['includesubcategory'])) {
			$sql_where .= " AND categoryup ='{$params['rootcategory']}'";
		} else {
			$categories = includesubcategories($params['rootcategory']);
			$sql_where .= " AND id IN ({$categories})";
		}
	}
...
function getlistsdata($params) {
	if($params['sourcetype'] == 'serialize') {
		$lists = unserialize($params['list']);
	} elseif($params['sourcetype'] == 'json') {
		$lists = json_decode($params['list'], 1);
	} else {
...
function getattachmentsdata($params) {
	...
	foreach($attachments as $attachment) {
		$j ++;
		$attachment['realid'] = $j + $params['start'] - 1;
...
function getpictures($params) {
	...
	if(isset($cache_memory[$params['cachekey']])) {
		...
		$_pics = $pics = array();
		if($params['sourcetype'] == 'spider' || $params['sourcetype'] == 'html') {
			...
			$_pics = $pics = array();
			if(isset($params['baseurl'])) $baseurl = $params['baseurl'];
			foreach($match[2] as $pic) {
				$pic = str_replace('"', '', $pic);
				$pic = str_replace('\'', '', $pic);
				$_pics[] = $pic;
			}
		} elseif($params['sourcetype'] == 'list') {
			$sc = isset($params['sc']) ? $params['sc'] : ',';
			$_pics = explode($sc, $params['source']);
			$baseurl = $homepage;
			if(isset($params['baseurl'])) $baseurl = $params['baseurl'];
		}
		foreach($_pics as $k => $pic) {
			if(!empty($params['character']) && strpos($pic, $params['character']) === false) unset($_pics[$k]);
			if(!empty($params['skip']) && strpos($pic, $params['skip']) !== false) unset($_pics[$k]);
			$_pics[$k] = calrealurl($pic, $baseurl);
		}
		$_pics = array_unique($_pics);
		$j = 1;
		foreach($_pics as $pic) {
			$item = array('picture' => $pic, 'id' => $j);
			if(strpos($params['template'], '[thumb:') !== false) {
				$_picture = getfield('[thumb:', ']', $params['template']);
				if(strpos($_picture, '*') !== false) {
					list($w, $h) = explode('*', $_picture);
...
function akecho($params) {
	...
	$starttime = akmicrotime();
	$html = $params['source'];
	if(empty($params['noreg'])) $html = regtext($html);
...
function operateparams($type, $params) {
	...
	foreach($params as $key => $value) {
		if($key == 'sql' || $key == 'where' || $key == 'emptymessage' || $key == 'overflow' || $key == 'startwith' || $key == 'endwith' || strpos($key, 'template') !== false) continue;
	...
	$startwith = empty($params['startwith']) ? '' : $params['startwith'];
	$endwith = empty($params['endwith']) ? '' : $params['endwith'];
	$emptymessage = str_replace('()', '"', $emptymessage);
	$startwith = str_replace('()', '"', $startwith);
	$endwith = str_replace('()', '"', $endwith);
	...
	if($type == 'items') {
		...
		$return['fields'] = array_unique($match[1]);
	} elseif($type == 'paging') {
...
function order_operate($rule, $type = 'items') {
	...
	foreach($rules as $rule) {
		...
		if(!isset($$arrayname) || !array_key_exists($array_temp[0], $$arrayname)) {
			$k = $array_temp[0];
		} else {
			$_arrayname = $$arrayname;
			$k = $_arrayname[$array_temp[0]];
		}
...
function getpaging($params) {
	...
	if(!isset($params['baseurl'])) {
		$urlinfo = parse_url($params['currenturl']);
		if(preg_match("/([?&]page=)([0-9]+)/", $urlinfo['query'])) {
			$query = preg_replace("/([?&]page=)([0-9]+)/", "$1[page]", $urlinfo['query']);
		} else {
			$query = $urlinfo['query']."&page=[page]";
		}
		$params['baseurl'] = $urlinfo['path'].'?'.$query;
	}
...
function assign($params) {
	$assign = array();
	foreach($params as $key => $value) {
		$assign[$key] = regtext($value);
	}
	$GLOBALS['tpl']->assign($assign);
}
?>
