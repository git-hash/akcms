...
function getitems_parseflag($params) {
	$template = $params['template'];
	foreach($params as $key => $value) {
		if(!preg_match("/^template_([0-9]+)$/", $key)) continue;
		$template .= $value;
	}
	$f_ext = $f_data = $f_last = $f_titlekeywords = $f_titlehighlight = $f_datahighlight = $f_picture = false;
	if(preg_match("/\[%?_/", $template)) $f_ext = true;
	if(preg_match("/\[data/", $template)) $f_data = true;
	if(preg_match("/\[last_/", $template)) $f_last = true;
	if(preg_match("/\[title_keywords\]/", $template)) $f_titlekeywords = true;
	if(preg_match("/\[title_highlight\]/", $template)) $f_titlehighlight = true;
	if(preg_match("/\[data_highlight\]/", $template)) $f_datahighlight = true;
	if(preg_match("/\[%?picture/", $template)) $f_picture = true;
	return array(
		'f_ext' => $f_ext,
		'f_data' => $f_data,
		'f_last' => $f_last,
		'f_titlekeywords' => $f_titlekeywords,
		'f_titlehighlight' => $f_titlehighlight,
		'f_datahighlight' => $f_datahighlight,
		'f_picture' => $f_picture
	);
}

function getitems_parsetemplate($itemdata, $params) {
	$template = $params['template'];
	$module = $itemdata['module'];
	if(isset($params['template_'.$module])) $template = $params['template_'.$module];
	return $template;
}
...
function db($config = array(), $forcenew = 0) {
	...
	if(empty($config)) {
		global $dbname, $dbhost, $dbuser, $dbpw, $charset, $utf8mb4;
		if($dbtype == 'mysql') {
			$config['type'] = 'mysql';
			...
			if(!empty($utf8mb4)) $config['utf8mb4'] = 1;
...
function render_template($template, $pagevariables = array(), $createhtml = 0, $show = 0) {
	...
	global $template_path, $tpl, $user, $thetime, $lan, $lr, $header_charset, $sections, $setting_storemethod, $homepage, $setting_defaultfilename, $sysname, $sysedition, $settings;
	...
	$function = 'akinclude,akincludeurl,getitems,getsections,gettexts,getcategories,getcomments,getlists,monitor,getindexs,getpaging,ifhassubcategories,getattachments,getkeywords,getsqls,getinfo,getuser,getpictures,akecho,gettime,assign';
	...
	$_hook = actionhookfile('foretemplate');
	if(file_exists($_hook)) include $_hook;
...
function get_item_data($id, $template = '', $params = array(), $uid = 0) {
	global $template_path, $db, $globalvariables, $tablepre, $lan, $thetime, $system_root, $lr, $setting_homepage, $setting_ifhtml, $sections, $setting_storemethod, $setting_richtext, $homepage, $attachurl;
	$globalvariables = getcache('globalvariables');
	...
	$section = (isset($item['section']) && !empty($sections[$item['section']])) ? $sections[$item['section']] : array();
	...
	$variables['data'] = render_item_data($variables['data']);
	...
	$variables['module'] = $item['module'];
	$variables['tags'] = $item['tags'];
	$_htmlname = itemhtmlname($item['id'], 1, $item, $categorycache);
	$variables['htmlfilename'] = FORE_ROOT.$_htmlname;
	$variables['currenturl'] = $_htmlname;
	if($item['category'] > 0) {
		for($i = 2; $i <= 4; $i ++) {
...
function calrealurl($target, $baseurl = '') {
	...
	if(strpos($target, '//') === 0) return 'http:'.$target;
...
function if_template_exists($template, $main = 1) {
	global $template_path;
	if($main == 1) $template = ','.$template;
	$template_filename = AK_ROOT."configs/templates/$template_path/$template";
	return file_exists($template_filename);
}
?>