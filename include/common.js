...
function popup(url) {
	//弹出浮动窗体
	var iframehtml = "<div id='popup_div' style='position:absolute;left:-10000px;top:-10000px;z-index:100;border:6px solid #DDDDDD;overflow-x:hidden'><iframe id='popup_iframe' frameborder='0' scrolling='no'></iframe></div><a id='popup_close' style='z-index:101;float:right;display:block;position:absolute;width:26px;height:26px;top:0px;right:0px;background:url(images/admin/close.gif);background-repeat:no-repeat;background-position:center;' href='#'></a>";
	...
	$("#popup_iframe").load(function(){
		...
		if(height > windowHeight - 50) {
			$("#popup_div").css("overflow", "scroll");
		}
		height = Math.min(height, windowHeight - 50);
...
function closeparentajaxdiv(refresh) {
	//在小窗中关掉自身
	parent.$("#popup_close").remove();
	parent.$("#popup_div").remove();
	if(refresh != null) {
