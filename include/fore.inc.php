...
if(!securitycheck()) {
	...
	aexit($lan['crackwarning']."<script>setTimeout(\"location='http://www.akhtm.com/manual/crack-warning.htm?source=crack'\", 20000);</script>");
}
$foreflag = 1;
$db = db();
...
function securitycheck() {
	...
	foreach($_GET as $key => $value) {
		...
		if(preg_match('/<\//i', $value)) return false;
		if(preg_match('/&#/i', $value)) return false;
